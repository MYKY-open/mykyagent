#!/usr/bin/env bash
# Offline smoke test for /model-web: command handler, webmodel.json state,
# and that the override model reaches the distillation sub-call.
...
"$ESBUILD" "$DIR/model_web_smoke.ts" \
